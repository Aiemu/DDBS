...
class MongoInit:
    ...
    def init_article(self):
        ...
        if bulk_counter > 0:
            Article.bulk_write(bulk_dict)
        article_file.close()
        print('Load ARTICLE successfully!')

    def init_read(self):
        print('Start loading READ...')
        ...
        for line in read_file.readlines():
            ...
            if bulk_counter == self.bulk_size_read:
                regions = DDBSCore.query_user({'uid': {'$in': uids}}, {'uid': 1, 'region': 1}, cache=False)
        ...
        if bulk_counter > 0:
            ...
            Read.bulk_write({'Beijing': list(np.array(bulk_list)[beijing_index]),
                             'Hong Kong': list(np.array(bulk_list)[hk_index])})
        read_file.close()
        print('Load READ successfully!')

    def init_be_read(self):
        print('Start loading BE_READ...')
        bulk_counter, bulk_list, bulk_aids = 0, [], []
        aids = DDBSCore.query_article({}, {'aid': 1}, cache=False)
        aids = set(map(lambda x: x['aid'], aids))
        res = DDBSCore.query_read({'aid': {'$in': list(aids)}}, {'aid': 1, 'uid': 1}, cache=False)
        read_num, read_list = self.iterate_query_be_read(res)
        res = DDBSCore.query_read({'aid': {'$in': list(aids)}, "commentOrNot": '1'}, {'aid': 1, 'uid': 1}, cache=False)
        comment_num, comment_list = self.iterate_query_be_read(res)
        res = DDBSCore.query_read({'aid': {'$in': list(aids)}, "agreeOrNot": '1'}, {'aid': 1, 'uid': 1}, cache=False)
        agree_num, agree_list = self.iterate_query_be_read(res)
        res = DDBSCore.query_read({'aid': {'$in': list(aids)}, "shareOrNot": '1'}, {'aid': 1, 'uid': 1}, cache=False)
        ...
        for aid in aids:
            ...
            bulk_list += [InsertOne(doc)]
            if bulk_counter == self.bulk_size:
                categories = DDBSCore.query_article({'aid': {'$in': bulk_aids}}, {'category': 1}, cache=False)
        ...
        if bulk_counter > 0:
            ...
            BeRead.bulk_write({'science': list(np.array(bulk_list)[science_index]),
                               'technology': list(np.array(bulk_list)[tech_index])})
        print('Load BE_READ successfully!')
    ...
    def init_popular_rank(self):
        print('Start loading POPULAR_RANK...')
        read_file = open(self.root_dir + self.read_path, 'r')
        ...
        print('Load POPULAR_RANK successfully!')


if __name__ == '__main__':
    mongo_init = MongoInit()
    print('=' * 30)
    mongo_init.init_user()
    print('-' * 30)
    mongo_init.init_article()
    print('-' * 30)
    mongo_init.init_read()
    print('-' * 30)
    mongo_init.init_be_read()
    print('-' * 30)
    mongo_init.init_popular_rank()
    print('=' * 30)